...
3. **Qualify every object reference *inside* the macro — the hidden-window problem has a second face.** `Sheets("X")`, `Range("A1")`, `Cells(...)`, `ActiveSheet`, `ActiveWorkbook` all resolve through VBA's `_Global`, which needs an **active** workbook/sheet. With none (hidden-window loader): `Sheets("X")` fails with runtime error **1004** (`method 'Sheets' of object '_Global' failed`), and `Set ws = ActiveSheet` returns `Nothing` **without raising** — the failure only appears on the *next* line that uses `ws` (**91**, object variable not set). Running the macro by hand always works, which is exactly why such landmines survive for years, while **every automated call fails**. Always write `ThisWorkbook.Worksheets("X")` or an explicit workbook/sheet variable; find every offender in a source tree with `python tools/vba/vba_lint_refs.py <dir-or-file>` (static, read-only, handles GBK/UTF-8 sources). **Third face — `Application.Calculation`.** Same-file A/B (`python tools/vba/hidden_app_props_probe.py`): with a hidden window `ScreenUpdating` / `DisplayAlerts` / `StatusBar` / `EnableEvents` and qualified cell writes all behave normally — **only `Application.Calculation = …` (read *or* write) raises `1004 application-defined or object-defined error`**. It is a speed switch, not correctness: wrap every read/write of it (`On Error Resume Next`, fall back to automatic) so a performance toggle can never abort the chain — `run_vba.py` still forces `CalculateFull` before evaluating assertions.
