...
3. **Qualify every object reference *inside* the macro — the hidden-window problem has a second face.** `Sheets("X")`, `Range("A1")`, `Cells(...)`, `ActiveSheet`, `ActiveWorkbook` all resolve through VBA's `_Global`, which needs an **active** workbook/sheet. With none (hidden-window loader): `Sheets("X")` fails with runtime error **1004** (`method 'Sheets' of object '_Global' failed`), and `Set ws = ActiveSheet` returns `Nothing` **without raising** — the failure only appears on the *next* line that uses `ws` (**91**, object variable not set). Running the macro by hand always works, which is exactly why such landmines survive for years, while **every automated call fails**. Always write `ThisWorkbook.Worksheets("X")` or an explicit workbook/sheet variable; find every offender in a source tree with `python tools/vba/vba_lint_refs.py <dir-or-file>` (static, read-only, handles GBK/UTF-8 sources).
4. **Never emit `MsgBox` / `InputBox` / `Stop` / `Debug.Assert` / UserForm `.Show`.** Each one hangs an automated instance (`Stop` / `Debug.Assert` are the sneakiest: they drop the VBE into break mode → **no dialog, pure hang**). `run_vba.py` blocks them before injection.
...
7. **Performance**: compute inside arrays and **write the range in one shot**; disable `ScreenUpdating` and set `Calculation = xlCalculationManual` (restore afterwards); prefer formulas / SUMIF / PivotTables over VBA loops.
