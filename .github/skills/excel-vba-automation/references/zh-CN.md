...
1. **注入前剥掉所有 `Attribute` 行**。`Attribute VB_Name = "X"` 是 VBE 自己的元数据，只在导出的 `.bas` 里合法；当源码注入会让该模块编译失败，**并且拖垮整个工程**（旁边新加的干净模块也跑不了），Excel 报 `0x800A03EC` + "宏可能被禁用" —— 极易误判成宏安全设置。`run_vba.py` 会自动剥；手写注入时要自己剥，且**坏模块必须就地覆写或删除**，旁边加新模块无效。
   - ⚠️ **但改「已有业务模块」时反过来要小心**：导出 → 剥 Attribute → 整模块回写的做法会**连带丢掉过程级属性** —— 例如 `Attribute 过程名.VB_ProcData.VB_Invoke_Func = "P\n14"` 就是**快捷键绑定**。实测一个 1874 行的生产模块被这样"优化"后，该宏的快捷键静默失效（改的人只验证了运行结果，所以没发现）。规则：**剥 Attribute 只针对要新注入的代码**；动已有模块前先导出 diff，把 `VB_ProcData.*` 这类过程属性行原样带过去。恢复手法：把该行插回导出的 `.bas` → `VBComponents.Remove` → `Import`（**Import 接受 Attribute 行，只有 `AddFromString` 会炸**），文件必须按系统 ANSI(cp936) 写，否则中文注释全毁。
